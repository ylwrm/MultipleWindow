...
namespace System.Windows
{
    public class Win32WindowEvents
    {
        ...
        static void WinEventProc(IntPtr hWinEventHook, uint eventType, IntPtr hwnd, int idObject, int idChild, uint dwEventThread, uint dwmsEventTime)
        {
            ...
            {
                var predicateX = predicates[predicates.Count-i-1];
                var predicate = predicateX.Item1;
                //if window is found fire event
                if (predicate != null && predicate.Invoke(Window) == true)
                {
                    predicateX.Item2(Window.Process, Window, (EventTypes)eventType);
                    if (!predicateX.Item3)
                    {
                        predicates.Remove(predicateX);
                    }
            ...
            foreach (var predicateX in predicates)
            {
            }
        ...
        static List<Tuple<Func<Win32Window, bool>, WinEvent, bool>> predicates = new List<Tuple<Func<Win32Window, bool>, WinEvent, bool>>();

        public static void WaitForWindowWhere(Func<Win32Window, bool> Predicate, WinEvent WinEvent, bool continueListen = false)
        {
            predicates.Add(Tuple.Create(Predicate, WinEvent, continueListen));
